...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <form action="/posts/{{ $post->id }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="file"  name="post[image_path]"><label>※画像を変更する場合はファイルを選択してください。</label>
            ...
            <div class="name">
                ...
                <input type="text" name="post[name]" value="{{ old('post.name', $post->name) }}"/>
            ...
            <div class="infomation">
                ...
                <div class="sex">
                    ...
                    <option value="">選択してください</option>
                        @foreach ($sexes as $sex)
                                <option value="{{ $sex->id }}" @if (old('post.sex_id',$post->sex_id) == $sex->id) selected @endif>{{ $sex->sex }}</option>
                        @endforeach
                    </select>
                ...
                <div class="breed">
                    ...
                    <option value="">選択してください</option>
                        @foreach ($breeds as $breed)
                                <option value="{{ $breed->id }}" @if (old('post.breed_id',$post->breed_id) == $breed->id) selected @endif>{{ $breed->breed }}</option>
                        @endforeach
                    </select>
                ...
                <div class="area">
                    ...
                    <option value="">選択してください</option>
                        @foreach ($areas as $area)
                                <option value="{{ $area->id }}" @if(old('post.area_id', $post->area_id)== $area->id) selected @endif>{{ $area->area }}</option>
                        @endforeach
                    </select>
                ...
                <div class="size">
                    ...
                    <option value="">選択してください</option>
                        @foreach ($sizes as $size)
                                <option value="{{ $size->id }}" @if(old('post.size_id', $post->size_id)== $size->id) selected @endif>{{ $size->size }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="post[age]" value="{{ old('post.age', $post->age) }}"/>
                    <p class="age__error" style="color:red">{{ $errors->first('post.age') }}</p>
                </div> 
                <div>
                    <label for="backgrooud">募集経緯</label>
                    <textarea name="post[backgrooud]" placeholder="里親を募集することになった経緯を入力">{{ old('post.backgrooud', $post->backgrooud) }}</textarea>
                     <p class="backgrooud__error" style="color:red">{{ $errors->first('post.backgrooud') }}</p>
                </div>
                <div>
                    <label for="personality">性格・特徴</label>
                    <textarea name="post[personality]" placeholder="性格や特徴を入力">{{ old('post.personality', $post->personality) }}</textarea>
                    <p class="personality__error" style="color:red">{{ $errors->first('post.personality') }}</p>
                </div>
                <div>
                    <label for="condition">引き渡し条件</label>
                    <textarea name="post[condition]" placeholder="引き渡しの条件を入力">{{ old('post.condition', $post->condition) }}</textarea>
                     <p class="condition__error" style="color:red">{{ $errors->first('post.condition') }}</p>
                </div>
                <div>
                    <label for="adopt">引き渡し方法</label>
                    <textarea name="post[adopt]" placeholder="引き渡し方法入力">{{ old('post.adopt', $post->adopt) }}</textarea>
                     <p class="adopt__error" style="color:red">{{ $errors->first('post.adopt') }}</p>
                </div>
                <div>
                    <label for='period'>締切日</label>
                    <input type="date" name="post[period]" value="{{ old('post.period', $post->period) }}">
